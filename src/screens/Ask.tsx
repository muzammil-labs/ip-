...
import { performAsk, citeNumbering } from "../engines/ask";
import { computeConfidence } from "../engines/confidence";
import { LVL_KEY } from "../data/constants";
...
const PERSONAS: { key: ReturnType<typeof useAppType>["persona"]; labelKey: string }[] = [
    ...
    <div className="mx-auto max-w-[1000px] px-4 py-10 sm:px-6 sm:py-14">
      ...
      {history.length > 1 && (
        <div className="mt-4 flex flex-wrap gap-2">
          <span className="text-[12px] text-ink-3">{t("earlier")}</span>
            ...
            {current.abstain ? (
              <div className="mt-3 rounded-lg border border-line bg-surface p-6">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h3 className="text-[16px] font-bold">{t("abstainTitle")}</h3>
                    <p className="mt-1.5 text-[14px] leading-relaxed text-ink-2">{current.abstain.why}</p>
                    <h4 className="mt-3 text-[13.5px] font-semibold">{t("abstainWhatYouCanDo")}</h4>
              ...
              <>
                {confidence && (
                  <div className="mt-3 rounded-lg border border-line bg-surface p-4">
                    <p className="text-[14px] font-bold text-ink">
                      {{ 3: t("confHigh"), 2: t("confMod"), 1: t("confLow") }[confidence.overall]}
                    </p>
                    <div className="mt-2 grid gap-1.5 text-[12.5px] sm:grid-cols-3">
                      <div className="flex items-center gap-2"><span className="text-ink-3">{t("confAuthority")}</span><ConfidenceBars level={confidence.auth} /><span>{t(LVL_KEY[confidence.auth])}</span></div>
                      <div className="flex items-center gap-2"><span className="text-ink-3">{t("confCoverage")}</span><ConfidenceBars level={confidence.cov} /><span>{t(LVL_KEY[confidence.cov])}</span></div>
                      <div className="flex items-center gap-2"><span className="text-ink-3">{t("confAgreement")}</span><ConfidenceBars level={confidence.agr} /><span>{confidence.agr === 3 ? t("confConsistent") : (confidence.conf === 1 ? t("confDisputeSing") : t("confDisputePlur").replace("{n}", String(confidence.conf)))}</span></div>
                    </div>
                  </div>
                )}
                ...
                {current.gaps && (
                  <div className="mt-4 rounded-lg border border-line bg-surface p-5">
                    <h4 className="text-[13.5px] font-bold text-ink">{t("gapsTitle")}</h4>
